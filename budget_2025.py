...
#%%
 ...
 #%% Sankey Numeric
sankey_nodes = [
    'Profit Tax 利得稅', 'Salaries Tax 薪俸稅', 'Stamp Duty 印花稅', 'Property Tax 物業稅', 'Other Internal Revenue 其他內部稅收', 'Other Operating Revenue 其他經營收入',
    'Capital Revenue 非經營收入', 'Transfers from Funds 從各基金轉撥的款項', 'Government Bond 政府債券', 'Deficit 赤字',
    'Revenue 收入',
    'Social Welfare 社會福利', 'Infrastructure 基建', 'Health 醫療', 'Education 教育', 'Security 安全', 'Economics 經濟', 'Environment & Food 環境與食物', 'Others 其他', 
    'Subventions 資助金', 'Personal Emoluments 個人薪酬', 'Departmental Expenses 部門開支', 'Personnel Related Expenses 與員工有關連的開支', 'Other Charges 其他費用',
    'Capital Investment Fund 資本投資基金', 'Innovation and Technology Fund 創新及科技基金', 'Government Bonds - Interest & Repayment 政府債券-利息及還款', 
...
sankey_node_colours = [
    'limegreen', 'limegreen', 'limegreen', 'limegreen', 'limegreen', 'lightgreen',
    'thistle', 'thistle', 'lightsalmon', 'tomato',
    'skyblue',
    'pink', 'khaki', 'firebrick', 'royalblue', 'mediumvioletred', 'burlywood', 'green', 'grey',
    'teal', 'olive', 'mediumslateblue', 'darkolivegreen', 'grey',
    'khaki', 'aqua', 'red',
]

opacity = 0.3
sankey_node_colours = [
    f'rgba(50, 205, 50 {opacity})',   # limegreen
    f'rgba(50, 205, 50 {opacity})',   # limegreen
    f'rgba(50, 205, 50 {opacity})',   # limegreen
    f'rgba(50, 205, 50 {opacity})',   # limegreen
    f'rgba(50, 205, 50 {opacity})',   # limegreen
    f'rgba(144, 238, 144 {opacity})',  # lightgreen
    f'rgba(216, 191, 216 {opacity})',  # thistle
    f'rgba(216, 191, 216 {opacity})',  # thistle
    f'rgba(255, 160, 122 {opacity})',  # lightsalmon
    f'rgba(255, 99, 71 {opacity})',    # tomato
    f'rgba(135, 206, 235 {opacity})',  # skyblue
    f'rgba(255, 192, 203 {opacity})',  # pink
    f'rgba(240, 230, 140 {opacity})',  # khaki
    f'rgba(178, 34, 34 {opacity})',    # firebrick
    f'rgba(65, 105, 225 {opacity})',   # royalblue
    f'rgba(199, 21, 133 {opacity})',   # mediumvioletred
    f'rgba(222, 184, 135 {opacity})',  # burlywood
    f'rgba(0, 128, 0 {opacity})',      # green
    f'rgba(128, 128, 128 {opacity})',  # grey
    f'rgba(0, 128, 128 {opacity})',    # teal
    f'rgba(128, 128, 0 {opacity})',    # olive
    f'rgba(123, 104, 238 {opacity})',  # mediumslateblue
    f'rgba(85, 107, 47 {opacity})',    # darkolivegreen
    f'rgba(128, 128, 128 {opacity})',  # grey
    f'rgba(240, 230, 140 {opacity})',  # khaki
    f'rgba(0, 255, 255 {opacity})',    # aqua
    f'rgba(255, 0, 0 {opacity})'       # red
]

node_to_numeric_node = dict(zip(sankey_nodes, range(len(sankey_nodes))))
node_numeric_to_colour = dict(zip(range(len(sankey_nodes)), sankey_node_colours))

sankey_data_numeric = (sankey_data
    .assign(
        SourceNumeric = lambda x: x['Source'].map(node_to_numeric_node), 
        TargetNumeric = lambda x: x['Target'].map(node_to_numeric_node), 
        Colour = lambda x: x['SourceNumeric'].map(node_numeric_to_colour)
    )
)

sankey_data_numeric.to_csv(f'{DIR}/sankey_data_numeric.csv', index=False)
...
# dict by zip node and range of length
node_to_numeric_node = dict(zip(sankey_nodes, range(len(sankey_nodes))))
node_numeric_to_colour = dict(zip(range(len(sankey_nodes)), sankey_node_colours))


coordinates = [
    [0.01, 0.1], [0.01, 0.2], [0.01, 0.26], [0.01, 0.31], [0.01, 0.35], [0.01, 0.43], [0.01, 0.61], [0.01, 0.66], [0.01, 0.75], [0.01, 0.85],
    [0.33, 0.5],
    [0.5, 0.15], [0.5, 0.31], [0.5, 0.445], [0.5, 0.54], [0.5, 0.62], [0.5, 0.71], [0.5, 0.74], [0.5, 0.80],
    [0.8, 0.15], [0.8, 0.31], [0.8, 0.4], [0.8, 0.45], [0.8, 0.5], [0.8, 0.7], [0.8, 0.78], [0.8, 0.83],
    [0.99, 0.99]
]
...
fig = go.Figure(data=[go.Sankey(
    node = dict(
        ...
        label = sankey_nodes,
        color = sankey_node_colours,
        x = [x for x, y in coordinates],
        y = [y for x, y in coordinates],

    ),
    link = dict(
        source = sankey_data_numeric['SourceNumeric'].tolist(),
        target = sankey_data_numeric['TargetNumeric'].tolist(),
        value = sankey_data_numeric['Value'].tolist(),
        color = sankey_data_numeric['Colour'].tolist(),
    )
    )])
